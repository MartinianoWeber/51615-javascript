...
form1.addEventListener("submit", (event) => {
  ...
  validar == true && calcularPrestamo(); // Si validar es true, ejecuta calcularPrestamo
});

// NULISH
document.addEventListener("DOMContentLoaded", () => {
  let cotizacion = localStorage.getItem("cotizacion");
  const prueba = cotizacion ?? [];
  if (prueba.length) {
    console.log("hay algo");
    JSON.parse(cotizacion).forEach((cotizacion) => {
...
function cargarCotizaciones() {
  ...
  cotizaciones.length > 0
    ? cotizaciones.forEach((cotizacion) => {
        const { nombre: n, apellido, dni, cuota, interes } = cotizacion;
        containerDeCotizaciones.innerHTML += `<p>Nombre: ${n} - Apellido: ${apellido} - DNI: ${dni} - Cuota ${cuota} - Interes ${interes}</p>
